# Author: https://gist.github.com/901929
...
module Randomizer
  ...
  included do
    key :random, Float
    before_create Proc.new{ |doc| doc.random = rand }
  end
  ...
  module ClassMethods
    ...
    def random(*args)
      ...
      result = criteria.all(:random => { :$gte => number })

      criteria = criteria.limit(opts[:limit] - result.length)
      result += criteria.all(:random => { :$lte => number }) if result.length < opts[:limit]
      result
    end
    ...
    def really_random(*args)
      ...
      [].tap do |results|
        opts[:limit].times do
          number = rand
          results << where(opts[:selector]).first(:random => { :$gte => number }) ||
                     where(opts[:selector]).first(:random => { :$lte => number })
        end
      end
    end
  end
